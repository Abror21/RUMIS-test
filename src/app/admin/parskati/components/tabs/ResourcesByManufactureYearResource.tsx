import { ParsedResourceManufactureYearReport, ResourceManufactureYearReport, ResourceReportFilters, ResourceSocialSupportReport, ResourceUsagePurposeReport } from "@/app/types/Report";
import { mmDdYyFormat } from "@/app/utils/AppConfig";
import useQueryApiClient from "@/app/utils/useQueryApiClient";
import { createReportByManufactureYearResourceExcelFile, goToUrl } from "@/app/utils/utils";
...
const ResourcesByManufactureYearResource = () => {
    const [reportData, setReportData] = useState<ParsedResourceManufactureYearReport[]>([])
    ...
    const {
        ...
        request: {
            url: '/reports/resourcesByManufactureYearResource',
            data: filter,
        },
        onSuccess: (response: ResourceManufactureYearReport[]) => {
            const data: any[] = []
            ...
            response.map(item => {
                if (data?.some(i => i.educationalInstitutionId === item.educationalInstitution.id)) {
                    ...
                    if (data[index].types.some((s: any) => s.code === item.resourceType.code)) {
                        ...
                        if (data[index].types[typeIndex].years.some((y: any) => y.year === item.manufactureYear)) {
                            const yearIndex = data[index].types[typeIndex].years.findIndex((s: any) => s.year === item.manufactureYear)
                            data[index].types[typeIndex].years[yearIndex][item.resourceStatus.code] += item.resourceCount
                        } else {
                            data[index].types[typeIndex].years.push({
                ...
                <div className='overflow-auto'>
                    <Spin spinning={isLoading}>
                        <div className='flex justify-end mb-2'>
                            <Button type='primary' onClick={() => {createReportByManufactureYearResourceExcelFile(reportData)}}>Eksportēt sarakstu</Button>
                        </div>
                    </Spin>
                                    ...
                                    <React.Fragment key={`${data.educationalInstitutionName}-${index}`}>
                                        ...
                                        {data.types.map((type: any) => (
                                            <React.Fragment key={`${type.code}`}>
                                                <tr className="">
                                                    <td className="confluenceTd align-top" rowSpan={type.years.length + 1}><Link href={`/admin/resources?educationalInstitution=${data.educationalInstitutionId}`}><u>{type.value}</u></Link></td>
                                                ...
                                                {type.years.map((y: any) => (
                                                    <tr className="" key={y.year}>
                                                        <td className="confluenceTd text-end">{y.year}</td>
                                                        <td className="confluenceTd text-center">{y?.new ?? 0}</td>
                                                        <td className="confluenceTd text-center">{y?.available ?? 0}</td>
                                                        <td className="confluenceTd text-center">{y?.reserved ?? 0}</td>
                                                        <td className="confluenceTd text-center">{y?.in_use ?? 0}</td>
                                                        <td className="confluenceTd text-center">{y?.maintenance ?? 0}</td>
                                                        <td className="confluenceTd text-center">{y?.under_repair ?? 0}</td>
                                                    </tr>
                                                ))}
                                            </React.Fragment>
                                        ))}
...
export default ResourcesByManufactureYearResource