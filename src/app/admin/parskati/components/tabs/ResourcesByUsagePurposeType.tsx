import { ParsedResourceUsagePurposeReport, ResourceReportFilters, ResourceSocialSupportReport, ResourceUsagePurposeReport } from "@/app/types/Report";
import { mmDdYyFormat } from "@/app/utils/AppConfig";
import useQueryApiClient from "@/app/utils/useQueryApiClient";
import { createReportByUsagePurposeTypeExcelFile, goToUrl } from "@/app/utils/utils";
...
const ResourcesByUsagePurposeType = () => {
    const [reportData, setReportData] = useState<ParsedResourceUsagePurposeReport[]>([])
    ...
    const {
        ...
        request: {
            url: '/reports/resourcesByUsagePurposeType',
            data: filter,
        },
        onSuccess: (response: ResourceUsagePurposeReport[]) => {
            const data: any[] = []
            ...
            response.map(item => {
                if (data?.some(i => i.educationalInstitutionId === item.educationalInstitution.id)) {
                    ...
                    if (data[index].types.some((s: any) => s.code === item.resourceType.code)) {
                        ...
                        if (item.usagePurposeType.code === "issued_individually") {
                            data[index].types[typeIndex].issuedIndividually[item.resourceStatus.code] += item.resourceCount
                        } else {
                            data[index].types[typeIndex].institutionLearningProcess[item.resourceStatus.code] += item.resourceCount
                        }
                    } else {
                        data[index].types.push({
                            ...
                            issuedIndividually: {
                                ...
                                maintenance: 0,
                                under_repair: 0,
                                ...(item.usagePurposeType.code === "issued_individually"
                                    ? { [item.resourceStatus.code]: item.resourceCount }
                                    : {}),
                            },
                            institutionLearningProcess: {
                                ...
                                maintenance: 0,
                                under_repair: 0,
                                ...(item.usagePurposeType.code === "institution_learning_process"
                                    ? { [item.resourceStatus.code]: item.resourceCount }
                                    : {}),
                            }
                        })
                ...
                <div className='overflow-auto'>
                    <Spin spinning={isLoading}>
                        <div className='flex justify-end mb-2'>
                            <Button type='primary' onClick={() => {createReportByUsagePurposeTypeExcelFile(reportData)}}>Eksportēt sarakstu</Button>
                        </div>
                    </Spin>
                                    ...
                                    <React.Fragment key={`${data.educationalInstitutionName}-${index}`}>
                                        ...
                                        {data.types.map(type => (
                                            <React.Fragment key={`${type.code}`}>
                                                ...
                                                <tr className="">
                                                    <td className="confluenceTd text-end">Izsniegšanai individuāli</td>
                                                    <td className="confluenceTd text-center">{type?.issuedIndividually.new ?? 0}</td>
                                                    <td className="confluenceTd text-center">{type?.issuedIndividually.available ?? 0}</td>
                                                    <td className="confluenceTd text-center">{type?.issuedIndividually.reserved ?? 0}</td>
                                                    <td className="confluenceTd text-center">{type?.issuedIndividually.in_use ?? 0}</td>
                                                    <td className="confluenceTd text-center">{type?.issuedIndividually.maintenance ?? 0}</td>
                                                    <td className="confluenceTd text-center">{type?.issuedIndividually.under_repair ?? 0}</td>
                                                </tr>
                                                <tr className="">
                                                    <td className="confluenceTd text-end">Mācību procesam iestādē</td>
                                                    <td className="confluenceTd text-center">{type?.institutionLearningProcess.new ?? 0}</td>
                                                    <td className="confluenceTd text-center">{type?.institutionLearningProcess.available ?? 0}</td>
                                                    <td className="confluenceTd text-center">{type?.institutionLearningProcess.reserved ?? 0}</td>
                                                    <td className="confluenceTd text-center">{type?.institutionLearningProcess.in_use ?? 0}</td>
                                                    <td className="confluenceTd text-center">{type?.institutionLearningProcess.maintenance ?? 0}</td>
                                                    <td className="confluenceTd text-center">{type?.institutionLearningProcess.under_repair ?? 0}</td>
                                                </tr>
                                            </React.Fragment>
                                        ))}
                                    </React.Fragment>
...
export default ResourcesByUsagePurposeType