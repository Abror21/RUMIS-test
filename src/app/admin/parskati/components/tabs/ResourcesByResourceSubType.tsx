import { ParsedResourceReportFilters, ReportFilters, ResourceReport, ResourceReportFilters } from "@/app/types/Report";
import { mmDdYyFormat } from "@/app/utils/AppConfig";
import useQueryApiClient from "@/app/utils/useQueryApiClient"
import { createReportResourcesByResourceSubTypeExcelFile, goToUrl } from "@/app/utils/utils";
...
const ResourcesByResourceSubType = () => {
    const [reportData, setReportData] = useState<ParsedResourceReportFilters[]>([])
    ...
    const {
        ...
        request: {
            url: '/reports/resourcesByResourceSubType',
            data: filter,
        },
        onSuccess: (response: ResourceReport[]) => {
            const data: any[] = []
            ...
            response.map(item => {
                if (data?.some(i => i.educationalInstitutionId === item.educationalInstitution.id)) {
                    ...
                    if (data[index].types.some((s: any) => s.code === item.resourceType.code)) {
                        ...
                        if (data[index].types[typeIndex].subTypes.some((s: any) => s.code === item.resourceSubType.code)) {
                            const subTypeIndex = data[index].types[typeIndex].subTypes.findIndex((s: any) => s.code === item.resourceSubType.code)
                            data[index].types[typeIndex].subTypes[subTypeIndex][item.resourceStatus.code] += item.resourceCount
                        } else {
                            data[index].types[typeIndex].subTypes.push({
                ...
                <div className='overflow-auto'>
                    <Spin spinning={isLoading}>
                        <div className='flex justify-end mb-2'>
                            <Button type='primary' onClick={() => {createReportResourcesByResourceSubTypeExcelFile(reportData)}}>Eksportēt sarakstu</Button>
                        </div>
                    </Spin>
                                ...
                                <React.Fragment key={`${data.educationalInstitutionName}-${index}`}>
                                    ...
                                    {data.types.map(type => (
                                        <React.Fragment key={`${type.code}`}>
                                            <tr className="">
                                                <td className="confluenceTd align-top" rowSpan={type.subTypes.length + 1}><Link href={`/admin/resources?resourceType=${type.id}`}><u>{type.value}</u></Link></td>
                                            ...
                                            {type.subTypes.map(s => (
                                                <tr className="" key={s.value}>
                                                    <td className="confluenceTd text-end">{s.value}</td>
                                                    <td className="confluenceTd text-center">{s?.new ?? 0}</td>
                                                    <td className="confluenceTd text-center">{s?.available ?? 0}</td>
                                                    <td className="confluenceTd text-center">{s?.reserved ?? 0}</td>
                                                    <td className="confluenceTd text-center">{s?.in_use ?? 0}</td>
                                                    <td className="confluenceTd text-center">{s?.maintenance ?? 0}</td>
                                                    <td className="confluenceTd text-center">{s?.under_repair ?? 0}</td>
                                                </tr>
                                            ))}
                                        </React.Fragment>
                                    ))}
...
export default ResourcesByResourceSubType